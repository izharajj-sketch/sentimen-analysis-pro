...
uploaded_kamus = st.sidebar.file_uploader("Upload Kamus Kata Baku, Jika Ada:", type=['xlsx', 'xls'])
if uploaded_kamus is not None:
    try:
        ...
        if 'tidak_baku' in kamus_data.columns and 'kata_baku' in kamus_data.columns:
            ...
            st.sidebar.error("❌ Excel harus memiliki kolom 'tidak_baku' and 'kata_baku'!")
...
# --- 5: Pelatihan Model (Hanya Split Data & Latih Model) ---
elif menu == "🧠 Pelatihan Model":
    st.title("🧠 Pemodelan Regresi Linear Sederhana")
    ...
    if len(df) < 5:
        ...
        if st.button("🚀 Train Data & Simpan Model", use_container_width=True):
            ...
            # Simpan hasil latihan ke session state agar bisa diakses menu evaluasi baru
...
# --- 6: Menu Baru Evaluasi Regresi & Grafik ---
    ...
    # PROTEKSI: Kunci akses jika model belum dilatih di Menu 5
    if "model" not in st.session_state:
        ...
        # --- GRAFIK LINEAR REGRESI SEDERHANA YANG IDEAL ---
        ...
        # 1. Menampilkan data aktual sebagai scatter plot
        ...
        # 2. Plot garis lurus regresi linear sederhana (Y = mX + c)
...
# --- 7: Export Data ---
    ...
    st.title("💾 Export Hasil Analisis Sentimen")
    df = st.session_state.df
    
    if not df.empty:
        st.success("Data tersedia untuk diunduh.")
    else:
        st.warning("Belum ada data hasil pemrosesan untuk di-export.")